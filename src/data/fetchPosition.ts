...
export const fetchPositions = async (date: Date, ship: Ship): Promise<ShipPosition> => {
  ...
  if (date.getDate() == today.getDate() && date.getMonth() == today.getMonth() && date.getFullYear() == today.getFullYear()) {
    const data = await fetchFromBarentsWatchForToday(ship);
    return data;
  }

  const data = await fetchFromBarentsWatchForDate(date, ship);
  return data;
};

async function fetchFromBarentsWatchForToday(ship: Ship): Promise<ShipPosition> {
  try {
...
async function fetchFromBarentsWatchForDate(date: Date, ship: Ship): Promise<ShipPosition> {
  try {
...
async function fetchFromKystdatahuset(date: Date, ship: Ship): Promise<ShipPosition> {
  try {
    ...
    const responseDayOne = await fetch('https://kystdatahuset.no/ws/api/ais/positions/for-mmsi-date', {
      ...
      body: JSON.stringify({
        mmsi: ship.mmsi,
        date: parseDateToKartverketTimeFormat(day1),
      }),
    });
    ...
    const responseDayTwo = await fetch('https://kystdatahuset.no/ws/api/ais/positions/for-mmsi-date', {
      ...
      body: JSON.stringify({
        mmsi: ship.mmsi,
        date: parseDateToKartverketTimeFormat(day2),
      }),
    });
